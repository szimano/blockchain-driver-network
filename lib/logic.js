...
async function acceptedFine(acceptedFine) {
    ...
    const allFines = await query('selectAcceptedFinesByDriver', 
        {driver: `resource:com.softwaremill.drivernetwork.Driver#${acceptedFine.fine.driver.licenseId}`});

    // the just updated acceptedFine.fine will be not yet reflected in the query, so we need to add it manually
    // https://stackoverflow.com/questions/50857955/hyperledger-composer-calling-a-query-within-a-transaction-that-updated-an-asset
    if (allFines.reduce((acc, val) => {return acc + val.penaltyPoints;}, 0) + acceptedFine.fine.penaltyPoints >= 21) {
        console.log('Driver excceded 21 points, blocking license');
